...
The difference between a `rowtype` and a `field` is that a single `rowtype` *must* be specified whereas one or more additional `fields` are optional, to break the measure down further.

## An Example
This example will fetch *HE students Full-person equivalents* (the `valuetype`) by *Institution* (the `rowtype`) and *JACS subject area* (a `field`). This example only retrieves one year of data for 2012/13.
...
### Setup
The first step is to run the `setup` function, supplying it with your organisation's unique identifier and HEIDI API key. Both should be available from the HEIDI local adminstrator's web interface, run `?setup` for more information. **The output of `setup` must be saved into a variable called `base64userpass`** as all of the `heidi` functions look for this exact variable name for the HEIDI authentication information.

```{r setup, eval=FALSE}
library(heidi)
base64userpass <- setup('1234', '1f1f1f1f-2e2e-3d3d-4c4c-5b5b5b5b5b5b')  # change to your details

# If you are behind a proxy server you may need to uncomment and run the code below
# set_config(use_proxy(url='yourproxy.ac.uk',8080))
